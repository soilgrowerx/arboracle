...
import { Search, Filter, SortAsc, Download, List, Map, Settings, BarChart3, BookOpen, Plus, Sprout } from 'lucide-react';
...
export default function Home() {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...trees.map(tree => {
        const treeAge = calculateTreeAge(tree.plantedDate);
        const ecosystemSpeciesCount = tree.ecosystemSpecies?.length || 0;
        const ecosystemCategories = tree.ecosystemSpecies ? [...new Set(tree.ecosystemSpecies.map(s => s.category))].join(';') : '';
        ...
        // Assuming calculateTreeAge now returns a string like "X years, Y months old"
        // We need to parse it to get a numeric age for scoring, or use a different metric.
        // For simplicity, let's use a placeholder for ageScore for now.
        const ageScore = 10; // Placeholder
        const ecosystemScore = Math.min(ecosystemSpeciesCount, 1) * 20;
        const dataScore = (tree.scientificName && tree.latitude && tree.longitude) ? 20 : 10;
        ...
        return [
          ...
          tree.longitude,
          `"${tree.plusCode}"`,
          `"${tree.plusCode}"`, // Assuming plusCode is both global and local for now
          `"${new Date(tree.plantedDate).toLocaleDateString()}"`,
          `"${treeAge}"`,
          `"${tree.verification_status}"`,
          `"${healthScore}%"`,
          ecosystemSpeciesCount,
          `"${ecosystemCategories}"`,
          `"${tree.seed_source || ''}"`,
          `"${tree.nursery_stock_id || ''}"`,
          `"${tree.conditionAssessment?.specificNotes || ''}"`, // Using specificNotes for summary
          `"${tree.managementActions || ''}"`,
  ...
  const getFilteredAndSortedTrees = () => {
    ...
    switch (sortBy) {
      ...
      case 'scientific-za':
        ...
        break;
      case 'age-oldest':
        sorted.sort((a, b) => new Date(a.plantedDate).getTime() - new Date(b.plantedDate).getTime());
        break;
      case 'age-youngest':
        sorted.sort((a, b) => new Date(b.plantedDate).getTime() - new Date(a.plantedDate).getTime());
        break;
      default:
        break;
    }
          ...
          <div className="flex flex-col gap-4 sm:gap-6 mb-2">
            ...
            <div className="mobile-stack">
              ...
              <div className="grid grid-cols-2 xs:grid-cols-4 gap-3 sm:gap-4">
                ...
                <Link href="/nursery" className="col-span-1">
                  <Button
                    variant="outline"
                    className="w-full btn-outline-enhanced border-yellow-300 text-yellow-700 hover:bg-gradient-to-r hover:from-yellow-50 hover:to-yellow-100 hover:border-yellow-400 touch-target mobile-button h-12 sm:h-14 font-semibold shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-300"
                  >
                    <div className="flex items-center justify-center gap-2">
                      <Sprout size={16} className="text-yellow-600" />
                      <span className="hidden xs:inline">🌱 Nursery</span>
                      <span className="xs:hidden text-xs font-bold">Nursery</span>
                    </div>
                  </Button>
                </Link>
                {trees.length > 0 && (
            ...
            {trees.length > 0 && (
              <Tabs value={activeView} onValueChange={setActiveView} className="w-full">
                ...
                <TabsContent value="map" className="space-y-6">
                  <TreeMapView filteredTrees={filteredTrees} onTreeSelect={handleTreeSelect} />
                </TabsContent>
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    >
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <span className="text-base sm:text-lg font-bold">+ Add Tree</span>
                      </div>
                    </Button>
